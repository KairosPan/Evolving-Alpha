...
- **Phase-1 分解**:✅**1a Agent(act)** → **1b 轨迹记录 + Refiner**(每日复盘→meta-tool 编辑 H,真正的自进化;agent 已能即时看到 H 变更)→ 1c 协同学习外环(PRM+teacher relabel+LoRA)。
- **Phase-1b/实盘债务(终审,非阻塞)**:① `DeepSeekClient` 无 retry/backoff——多日实盘 eval 前必加(否则单次 API 错误中断全程);② 提示注入全量(55技能+全doctrine,~24KB/次)未按 regime 选择性注入;③ win 类记忆 + 技能 stats/oracle_gap 未注入提示(Refiner 产出后 1b 补);④ 真实 DeepSeek 输出 + akshare 列名仅 smoke 验证,信任实盘数前先跑 `scripts/smoke_deepseek_agent.py`。

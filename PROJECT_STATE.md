...
- **Phase-1 分解**:✅**1a Agent(act)** → ✅**1b-1 观测层** → ✅**1b-2 LLM Refiner** → ✅**1b-3a 内环编排** → ✅**1b-3b 度量对比** → ✅**1b-3d Refiner 纪律化(退役证据门 + 提示纪律,治退化根因)** → ⏭ **1b-3c 影子 Hexpert 严格地板**(安全网)/ 或 **1c 协同学习外环**。**🔬 改进后复测已做(2026-06-07,findings §8)**:退役门**确实治住根因**——HCH 三窗 applied 里几乎无 retire(改用 patch/doctrine,技能库不再流失),机制层高置信;verdict 0/3→1/3、窗口1 强翻正(被 LLM 重采样污染,非定论)。**定量证明需多窗口/多 episode 聚合 + 控噪。**
...
- **roadmap**:✅0a/种子/0b/0c/0d 地基 → ✅1a Agent → ✅1b-1 观测层 → ✅1b-2 LLM Refiner → ✅1b-3a 内环编排 → ✅1b-3b 度量对比 → 🔬真实数据验:HCH 退化于 frozen(§6.9 复现)→ ✅1b-3d Refiner 纪律化(退役证据门治根因)→ ⏭ 1b-3c 影子严格地板 / 1c 协同学习 → 龙虎榜/题材线 + 仓位/组合层 + N日oracle/regime分层 等增强。底座(meta-tool+持久化+回滚+universe+评测尺+Refiner+内环+三方对比+退役纪律)全就绪;**自进化机器可跑可量;退役根因已治且复测确认行为改变(verdict 0/3→1/3,机制层高置信);定量结论待多窗口聚合。**

...
- **Phase-1 分解**:✅**1a Agent(act)** → ✅**1b-1 观测层** → ✅**1b-2 LLM Refiner** → ✅**1b-3a 内环编排** → ✅**1b-3b 度量对比(HCH/Hexpert/Hmin 同窗同 oracle + verdict,机器就绪)** → ⏭ **1b-3c 影子 Hexpert 严格地板**(环内并行冻结实时熔断)/ 或直接 **1c 协同学习外环**(PRM+teacher relabel+LoRA)。**关键待验**:真实 DeepSeek 多日跑下 HCH 是否真胜 Hexpert(bootstrap 不退化,蓝图 §6.9)。
...
- **roadmap**:✅0a/种子/0b/0c/0d 地基 → ✅1a Agent → ✅1b-1 观测层 → ✅1b-2 LLM Refiner → ✅1b-3a 内环编排 → ✅1b-3b 度量对比 → ⏭ 1b-3c 影子严格地板 / 1c 协同学习 → 龙虎榜/题材线 + 仓位/组合层 + N日oracle/regime分层 等增强。底座(meta-tool+持久化+回滚+universe+评测尺+Refiner+内环+三方对比)全就绪;**自进化内环可端到端跑 + 可同窗同 oracle 量(HCH vs Hexpert vs Hmin)**;真实实效待真实 DeepSeek 多日验证。

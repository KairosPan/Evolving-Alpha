...
- **Phase-1 分解**:✅**1a Agent(act)** → ✅**1b-1 观测层** → ✅**1b-2 LLM Refiner** → ✅**1b-3a 内环编排** → ✅**1b-3b 度量对比** → ⏭ **1b-3d Refiner 纪律化(治本,首推)** → **1b-3c 影子 Hexpert 严格地板**(安全网)→ 1c 协同学习外环(PRM+teacher relabel+LoRA)。**真实数据已证:朴素每日 refine + 小样本即编辑会退化到比 frozen 还差,先治本再上安全网。**
...
- **roadmap**:✅0a/种子/0b/0c/0d 地基 → ✅1a Agent → ✅1b-1 观测层 → ✅1b-2 LLM Refiner → ✅1b-3a 内环编排 → ✅1b-3b 度量对比 → 🔬**真实数据已验:HCH 退化于 frozen(§6.9 复现)** → ⏭ 1b-3d Refiner 纪律化(治本)→ 1b-3c 影子严格地板 → 1c 协同学习 → 龙虎榜/题材线 + 仓位/组合层 + N日oracle/regime分层 等增强。底座(meta-tool+持久化+回滚+universe+评测尺+Refiner+内环+三方对比)全就绪;**自进化机器完整可跑可量,但朴素自精炼会退化——下一步给 Refiner 上纪律。**

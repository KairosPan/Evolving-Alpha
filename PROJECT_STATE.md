...
- **Phase-1 分解**:✅**1a Agent(act)** → ✅**1b-1 观测层** → ✅**1b-2 LLM Refiner** → ✅**1b-3a 内环编排(交错环+在线信用+能力地板熔断,自进化环已可跑)** → ⏭ **1b-3b 度量对比**(HCH 自精炼 vs Hexpert 种子静态 vs Hmin 裸 baseline,同窗同 oracle;影子 Hexpert 严格地板;验证 bootstrap 不退化到比 frozen 还差,蓝图 §6.9)→ 1c 协同学习外环(PRM+teacher relabel+LoRA)。
...
- **roadmap**:✅0a/种子/0b/0c/0d 地基 → ✅1a Agent → ✅1b-1 观测层 → ✅1b-2 LLM Refiner → ✅1b-3a 内环编排 → ⏭ 1b-3b 度量对比(HCH/Hexpert/Hmin + 影子地板)→ 1c 协同学习 → 龙虎榜/题材线 + 仓位/组合层 + N日oracle/regime分层 等增强。底座(meta-tool+持久化+回滚+universe+评测尺+Refiner+内环)全就绪;**自进化内环已可端到端跑(act→打分→在线信用→refine,reset-free + 能力地板熔断)**。

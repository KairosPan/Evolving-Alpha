...
# 池制三类 + C3 收益尺两类一等公民:unfillable(选了买不到的票,如一字板)/missing(无 OHLCV)。
# unfillable/missing 仅 ReturnScorer 产出;池制 outcome()/path_outcome() 只返回前三类。
Outcome = Literal["continued", "faded", "nuked", "unfillable", "missing"]

SCORE: dict[str, float] = {"continued": 1.0, "faded": 0.0, "nuked": -1.0}

# C3:非成交结果——无真实交易,排除出信用分配/熔断证据/收益均值(由各消费方过滤)。
NON_TRADE_OUTCOMES: frozenset[str] = frozenset({"unfillable", "missing"})
